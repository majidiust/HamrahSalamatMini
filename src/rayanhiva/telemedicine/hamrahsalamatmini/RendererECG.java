...
public class RendererECG implements Renderer {
	private final int GAP_COUNT = 26;
	private final float DATA_LENGTH = 0.0225f;
	private float mTotalLength = 12.6f;
	private float mStartX = -6.3f;
	private final RectF AXIS_RECT;

	private FloatBuffer mVertexBuffer = null;
	private ShortBuffer mIndexBuffer = null;

	private FloatBuffer mGridBuffer = null;
	...
	public RendererECG(Context context, float width) {
		lock = 0;
		mTotalLength = width;
		mStartX = (mTotalLength / 2.0f) * -1.0f;
		AXIS_RECT = new RectF(-4.8f, 1.0f, -4.6f, -1.0f);
	}
	...
	private void setAllBuffers() {
		...
		for (int i = 0; i < mDataCount; ++i){
			...
			vertexlist[i * 3 + 1] = 0.0f;// (float)Math.sin((double)currentX * 2) / 2;
